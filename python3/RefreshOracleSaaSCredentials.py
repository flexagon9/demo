...
def updatedCredentialTextValue(base_url, password, cred_store_input_def_id, credential_id):
    ...
    if credential_patch_request.status_code == 200:
        credential_patch_resp = json.loads(credential_patch_request.content)
        credential_name = credential_patch_resp['credentialName']
        print(f"Successfully updated SaaS Credential {credential_name} for {base_url}, credential id {credential_id} and credential store input def id {cred_store_input_def_id}")
    else:
...
def update_saas_instance_passwords(level=0):
    ...
    print(f"Updating credential on production (fdtlp01) with credential id 16003")
    prodDev9CredInputDefId = getCredentialInputDefId('http://fdtlp01.flexagon.azure.com:8000', 16003, level=level+1)
    updatedCredentialTextValue('http://fdtlp01.flexagon.azure.com:8000', dev_9_pass, prodDev9CredInputDefId, 16003)
