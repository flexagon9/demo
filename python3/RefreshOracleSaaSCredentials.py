...
def getCredentialInputDefId(base_url, credential_id, level):
    credential_request = requests.get(f"{base_url}/flexdeploy/rest/v2/administration/security/credential/{credential_id}", auth=HTTPBasicAuth("${{FD_API_USER}}", "${{FD_API_PASSWORD}}"))
...
def updatedCredentialTextValue(base_url, password, cred_store_input_def_id, credential_id):
    ...
    credential_patch_request = requests.patch(f"{base_url}/flexdeploy/rest/v2/administration/security/credential/{credential_id}", headers=headers, data=payload, auth=HTTPBasicAuth("${{FD_API_USER}}", "${{FD_API_PASSWORD}}"))
